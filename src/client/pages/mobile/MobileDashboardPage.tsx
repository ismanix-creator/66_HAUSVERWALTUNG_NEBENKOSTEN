  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto flex max-w-xl flex-col gap-4 px-5 py-6">
        <header>
          <p className="text-xs uppercase tracking-widest text-slate-500">Read-Only Mobil-Ansicht</p>
          <h1 className="mt-1 text-2xl font-semibold text-slate-900">Mobile Übersicht</h1>
          <p className="text-sm text-slate-500">Alle Daten stammen vom Server, Schreibzugriffe sind deaktiviert.</p>
        </header>
        ...
        {data && (
          <>
            <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
              <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Schnellübersicht</h2>
            ...
            <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Offene Rechnungen</h2>
                <span className="text-xs text-slate-400">Top {data.outstandingInvoices.length}</span>
              </div>
              <div className="mt-3 space-y-3">
                ...
                {data.outstandingInvoices.map((invoice) => (
                  <article key={invoice.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
                    <div className="flex items-center justify-between text-sm text-slate-500">
                      <span>{invoice.rechnungsnummer ?? 'Rechnung'}</span>
                      <span>{formatDate(invoice.faelligkeitsdatum)}</span>
                    </div>
                    <p className="mt-1 text-lg font-semibold text-slate-900">{currencyFormatter.format(invoice.betrag)}</p>
                    <p className="text-xs text-slate-400">Ausgestellt: {formatDate(invoice.rechnungsdatum)}</p>
                  </article>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Erinnerungen</h2>
              ...
              <div className="mt-3 space-y-3">
                ...
                {data.reminders.map((reminder) => (
                  <article key={reminder.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
